'use strict';
//var debug = require('debug')('snailfwd:init'),
...
exports.desc = 'init snailfwd project';
exports.register = function (commander) {
    commander
        ...
        .action(function () {
            ...
            var projectAlias={
                '官网':'snailfwd-site-website',
                '手机官网':'snailfwd-site-mobliesite',
                '专题':'snailfwd-site-special',
            ...
            function remote(answers){
                ...
                var download = new Download({ extract: true, strip: 1, mode: '755' })
                    ...
                    .get('https://github.com/senro/' +projectAlias[answers.projectType] + '/archive/master.zip')
                    .dest('./')
                    .use(progress());
                ...
                rd.each('./', function (f, s, next) {
                    ...
                    if(stat.isFile()&&!fileIgnore.test(f)){
                        ...
                        if(answers.gameName&&snailGames[answers.gameName]){
                            content=content.replace(/\<\%title\%\>/g,snailGames[answers.gameName].title);
                            content=content.replace(/\<\%description\%\>/g,snailGames[answers.gameName].description);
                            content=content.replace(/\<\%keywords\%\>/g,snailGames[answers.gameName].keywords);
